  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.results}>
        ...
        {!query && <p className={styles.status}>Start typing to search.</p>}
        ...
        {tab === 'spaces' && results.map(space => (
          <Link key={space.id} href={`/spaces/${space.slug}`} className={styles.resultRow}>
            <div className={`${styles.avatar} ${styles.spaceIcon}`}>
              {space.icon_url
    ...
    <AppShell title="Search">
      <Suspense fallback={
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'60vh'}}>
          <span style={{width:8,height:8,borderRadius:'50%',background:'#c8ff00',display:'block'}} />
        </div>
      }>
        <SearchContent />
      </Suspense>
